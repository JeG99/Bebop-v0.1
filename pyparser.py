...
def tempCalculator(left_oper, right_oper, op):
    ...
    if right_oper in const_table.keys():
        ...
        if right_oper in func_dir[curr_scope]["vars_table"].keys():
            ...
            if right_oper >= 11000 and right_oper < 12000:
                rOperType = "int"
            elif right_oper >= 12000 and right_oper < 13000:
                rOperType = "float"
            rOperDir = right_oper
    ...
    if left_oper in const_table.keys():
        ...
        if left_oper in func_dir[curr_scope]["vars_table"].keys():
            ...
            if left_oper >= 11000 and left_oper < 12000:
                lOperType = "int"
            elif left_oper >= 12000 and left_oper < 13000:
                lOperType = "float"
            lOperDir = left_oper
...
def p_rsqrbracket_assign(p):
    ...
    dim = p[-2][2]
    if id not in func_dir[curr_scope]["vars_table"].keys():
        quadruples.append(["VERIFY", operands_stack[-1], 0,
                          func_dir["global"]["vars_table"][id]["lsDim1"]])
    else:
        quadruples.append(["VERIFY", operands_stack[-1], 0,
                          func_dir[curr_scope]["vars_table"][id]["lsDim1"]])
    quadCounter += 1
    aux = operands_stack.pop()
